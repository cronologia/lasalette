...
`cronologia/core` (skills: **sourcing-rules** — load
`/home/user/core/skills/sourcing-rules/SKILL.md` before touching any
`data/*.json` — plus bootstrap-project, data-edit, mine-video,
dossier-research); the architecture rationale in `cronologia/fsp` → `docs/adrs/`.

## What this project is

A compiled static website documenting the chronology of **the Marian
apparition reported at La Salette-Fallavaux on 19 September 1846**, the
Church's judgments on it, and the controversy over the "Secrets of La
Salette". A single JSON file is the source of truth; a zero-dependency Node
script compiles it into static HTML served by GitHub Pages.

**The data-edit gate is mandatory for every change to `data/*.json`:**

```
node scripts/validate-data.js && node --test && node build.js
```

then commit the data **and** the regenerated `docs/` in the same change.
...
The five core rules of the `sourcing-rules` skill (`cronologia/core`,
`/home/user/core/skills/sourcing-rules/SKILL.md`) govern every edit: cite it
or flag it as unverified; attribute, don't assert; sources span the spectrum;
time-sensitive statuses must be dated; testimony is a perspective, not a fact
source. Subject-specific rules for this repo:

1. **The apparition is a REPORTED event.** Everything about 19 September 1846
   rests on the two children's accounts. The dataset records who reported what
   and when, and what the Church ruled and when, citing the ruling document —
   it never asserts the supernatural claim as fact, and never denies it.
2. **Church-approval status is a critical field, and it is TWO fields.** The
   19 September 1851 pronouncement of Bishop de Bruillard (apparition worthy
   of belief) and the Holy Office acts of 21 December 1915 and 9 May 1923
   (discussion of the expanded "secret" restricted; Mélanie's booklet on the
   Index) are separate judgments about separate objects — the apparition vs.
   the expanded secrets. Never merge them into a single "approved"/"condemned"
   verdict; never let one silently revoke the other. The 1915 decree itself
   says devotion is not forbidden.
3. **The "Secrets of La Salette" are a RECORDED controversy, never resolved.**
   Which text is "the" secret — the sealed 1851 letters to Pius IX or the
   1879 Lecce booklet — is exactly what the parties dispute. Carry the
   dispute attributed and dated (`dateNote`); do not adjudicate it.
4. **Preferred primaries:** the sanctuary's official site (lasalette.cef.fr),
   diocesan documents, and the Acta Apostolicae Sedis scans on vatican.va
   (both Holy Office decrees were verified directly against the AAS PDFs —
   keep it that way for any new curial act).
5. **Search with `KEYWORDS.md`.** "La Salette" alone matches congregations,
   parishes and unrelated Italian confraternities; verify context before
   counting hits, and pair every zero with a positive control.
